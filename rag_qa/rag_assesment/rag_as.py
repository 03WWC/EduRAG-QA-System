...
with open(r"C:\Users\Administrator\Desktop\EduRAG\rag_qa\rag_assesment\rag_evaluate_data.json", "r", encoding="utf-8") as f:
...
# 3. 配置RAGAS评估环境（阿里云百炼）
DASHSCOPE_API_KEY = os.getenv("ALY_API_KEY")
DASHSCOPE_BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"

# LLM — 使用 ChatOpenAI 指向百炼兼容端点（RAGAS 需要 LangChain ChatModel）
llm = ChatOpenAI(
    model="qwen-plus",              # 百炼对话模型: qwen-plus / qwen-max / qwen-turbo
    openai_api_key=DASHSCOPE_API_KEY,
    base_url=DASHSCOPE_BASE_URL,
)

# Embeddings — 使用百炼原生 DashScope 嵌入模型
embeddings = DashScopeEmbeddings(
    model="text-embedding-v2",      # 百炼嵌入模型: text-embedding-v1 / v2 / v3
