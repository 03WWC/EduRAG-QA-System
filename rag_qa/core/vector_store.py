...
class VectorStore:
    ...
    def __init__(self,
        ...
        reranker_model_path = os.path.join(rag_qa_path, "model", "bge-reranker-large")
        self.reranker = CrossEncoder(reranker_model_path)
        
        # 初始化 BGE-M3 嵌入函数，使用 CPU 设备，不启用 FP16
        m3_model_path = os.path.join(rag_qa_path, "model", "bge-m3")
        self.embedding_function = BGEM3EmbeddingFunction(model_name_or_path=m3_model_path, use_fp16=False, device="cpu")
        # 获取稠密向量的维度
